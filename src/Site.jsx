...
export default function Site() {
  ...
  const brand = "Scaledy"; // change brand
    ...
    <header className={`sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-white/70 dark:supports-[backdrop-filter]:bg-slate-950/60 border-b border-slate-200/60 dark:border-slate-800/60 transition-shadow ${elevate ? "shadow-md shadow-slate-200/40 dark:shadow-black/20" : ""}`}>
      <div className="mx-auto max-w-7xl px-6 md:px-8 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex gap-6 text-sm">
          {nav.map((n) => (
            <a key={n.id} href={`#${n.id}`} className="hover:opacity-80 font-semibold">{lang === "en" ? n.en : n.mn}</a>
          ))}
        </nav>
      ...
      <AnimatePresence>
        {mobileOpen && (
          // <motion.aside initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="md:hidden border-t overflow-hidden">
...
function Plan({ name, tagline, usd, features, highlight, currency, USD_TO_MNT, lang }) {
  ...
  return (
    <div className={`rounded-2xl border p-6 flex flex-col bg-white/70 dark:bg-slate-900/40 backdrop-blur hover:shadow-2xl hover:-translate-y-0.5 transition ${highlight ? "ring-2 ring-indigo-600" : ""}`}>
      ...
      <div className="mt-3 text-3xl font-extrabold">{price}</div>e
      <ul className="mt-5 space-y-2 text-sm text-slate-600 dark:text-slate-300">
       ...
       <Link
        to="/apply"
        className={`mt-6 relative inline-flex items-center justify-center rounded-2xl px-4 py-2 font-medium ${highlight ? "bg-black text-white dark:bg-white dark:text-black" : "border"}`}
      >
